...
test_data[['id', 'score']].to_csv('res_lgb_0702_2.csv', index=False)
...
#与第一份数据融合
import pandas as pd
d1 = pd.read_csv('res_lgb_0702_2.csv')
d1['rank1'] = d1['score'].rank()
d2 = pd.read_csv('res_lgb_0702_1.csv')
d2['rank2'] = d2['score'].rank()
d = d1.merge(d2[['id','rank2']],on = 'id')
d['score'] = d['rank2']*0.8+d['rank1']*0.2

max_v = d['score'].max()
min_v = d['score'].min()
d['score'] = d['score'].apply(lambda x: (x - min_v) / (max_v - min_v))

d[['id', 'score']].to_csv('res_lgb_0702_3.csv', index=False)
